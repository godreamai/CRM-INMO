...
```
GODREAMAI/
├── cloude.md                    ← Este archivo. Fuente de verdad para la IA.
├── .Context/                    ← Contexto general del negocio
├── .Marca/                      ← Tono, estilo, guías de comunicación
├── .Plantillas/                 ← Contenido validado y templates de posts
│
├── clientes/
│   ├── template-cliente/        ← Copiar esto para cada cliente nuevo
│   │   ├── context.md           ← Brief completo del cliente
│   │   ├── sprint.md            ← Alcance, arquitectura, timeline, checklist
│   │   ├── propuesta.md         ← Propuesta comercial lista para enviar
│   │   └── notas-llamadas.md    ← Registro de diagnóstico y calls
│   │
│   └── [nombre-cliente]/        ← Una carpeta por cliente
│
└── proyectos-internos/
    ├── marketing/               ← Planes de contenido, ads, estrategia
    ├── ops/                     ← Automatizaciones y sistemas propios
    └── producto/                ← Mejoras al stack, dashboard, herramientas
```

**Convención de nombres para carpetas de clientes:** kebab-case, nombre de la empresa.
Ejemplo: `clientes/media-removal/`, `clientes/agencia-xyz/`

**Al abrir un proyecto de cliente:**
1. Leer `context.md` primero — contiene todo el contexto del proyecto.
2. Revisar `notas-llamadas.md` para entender el historial.
3. Consultar `sprint.md` para el estado técnico actual.
